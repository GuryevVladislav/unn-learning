...
def process_data(filepath_results, filepath_input, output_dir):
    """
    Считывает данные, создает DataFrame и сохраняет графики, таблицу и текстовый файл с результатами.
    """
    try:
        ...
        if name.lower() == "task":
            ...
            plt.plot(df['x2'], df['v2'], marker='s', linestyle='--', label='v2(x)')
            plt.xlabel('x')
            plt.ylabel('v, v2')
            plt.title(f'Графики v(x) и v2(x) для задачи: {name}, n: {n}')
            plt.grid(True)
            plt.legend()
            ...
            with open(output_text_filepath, 'w', encoding='utf-8') as text_file:
                ...
                text_file.write(f"Задача решена с точностью ε2 = {max_diff:.2e};\n")
        ...
        if name.lower() == "test":
            ...
            with open(output_text_filepath, 'w', encoding='utf-8') as text_file:
                ...
                text_file.write(
                    f"Максимальная разность численных решений в общих узлах сетки наблюдается в точке x = {max_x};")

    except FileNotFoundError:
        print("Один или оба файла не найдены.")
    except ValueError as e:
        print(f"Ошибка при чтении данных из input.txt или results.txt: {e}. Проверьте формат и кодировку файлов (UTF-8).")
